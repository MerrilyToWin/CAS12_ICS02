...
def run(train=True,model_name="model",epochs=50,steps=500,ard=False):
    ...
    if not train:
        state_dict = torch.load(f'models/{model_name}.bin', map_location=brain.Q_eval.device)
        filtered_state_dict = {k: v for k, v in state_dict.items() if k in brain.Q_eval.state_dict() and brain.Q_eval.state_dict()[k].shape == v.shape}
        brain.Q_eval.load_state_dict(filtered_state_dict, strict=False)
